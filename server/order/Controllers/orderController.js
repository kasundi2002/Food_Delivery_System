...
console.log("Creating order with data:", restaurantId);
    // 1. Create the order
    const newOrder = new Order({
      customerId:"681624cddf3ff8b66ac78ad7",
      restaurantId,
      items,
      totalAmount,
      deliveryLocation:{type:"Point", coordinates:[79.85105591298931, 6.90651170226483]},
      restaurantLocation:{type:"Point", coordinates:[79.86705779941985, 6.885107177697789]},
      paymentMethod,
    ...
    try {
      ...
      const deliveryServiceUrl =
        "http://localhost:3004/api/delivery/assign";
...
const getAssignedOrders = async (req, res) => {
  ...
  const deliveryProfile = await axios.get(
        "http://localhost:3004/api/delivery/profile",
        {
          headers: {
            Authorization: req.headers.authorization || "", // Forward JWT token if available
            "Content-Type": "application/json",
